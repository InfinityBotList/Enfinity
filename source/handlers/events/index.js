...
const loadEvents = async function (client) {
    const eventFolders = fs.readdirSync('./source/enfinity/events')

    for (const folder of eventFolders) {
        const eventFiles = fs.readdirSync(`./source/enfinity/events/${folder}`).filter(file => file.endsWith('.js'))

        for (const file of eventFiles) {
            const event = require(`../../enfinity/events/${folder}/${file}`)
...
const loadSlash = async function (client) {
    ...
    const commandFolders = fs.readdirSync('./source/enfinity/slash')

    for (const folder of commandFolders) {
        const commandFiles = fs.readdirSync(`./source/enfinity/slash/${folder}`).filter(file => file.endsWith('.js'))

        for (const file of commandFiles) {
            const command = require(`../../enfinity/slash/${folder}/${file}`)
...
const loadBase = async function (client) {
    fs.readdirSync('./source/enfinity/base').forEach(dir => {
        const commandFiles = fs.readdirSync(`./source/enfinity/base/${dir}/`).filter(f => f.endsWith('.js'))

        for (const file of commandFiles) {
            const command = require(`../../enfinity/base/${dir}/${file}`)
